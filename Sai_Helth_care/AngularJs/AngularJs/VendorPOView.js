app.service("VendorPOViewService", function ($http) {
    ...
    this.GetAccessoriesByProductId = function (id, custTypeId) {
        return $http({
            ...
            params: { id: id, CUSTOMER_TYPE_ID: custTypeId }
        });
    };
    ...
    this.Admin_Update = function (updatedProduct) {
        return $http({
            method: "POST",
            url: "/VendorPO/UpdateVendorPOProductAccessories",
            data: updatedProduct
        });
    };
...
app.controller("VendorPOViewCtrl", function ($scope, VendorPOViewService) {
    ...
    function GetAllSparepart() {
        if (!$scope.P_ID || !$scope.CUSTOMER_TYPE_ID) {
            console.warn("P_ID or CUSTOMER_TYPE_ID is undefined.");
            return;
        }

        // Show a loading indicator
        $scope.isLoading = true;
        $scope.ProductAccessoriesSparePartList = [];

        VendorPOViewService.GetAccessoriesByProductId($scope.P_ID, $scope.CUSTOMER_TYPE_ID).then(function (response) {
            $scope.ProductAccessoriesSparePartList = response.data;
            console.log("Spare parts fetched:", $scope.ProductAccessoriesSparePartList);
        }, function (error) {
            console.error("Error fetching spare parts:", error);
            // Optionally display an error message to the user
            $scope.errorMessage = "Failed to fetch spare parts. Please try again later.";
        }).finally(function () {
    ...
    $scope.updateProduct = function () {
        var updatedProduct = {
            VPO_ID: parseInt($scope.currentProduct.VPO_ID),
            VPO_P_ID: parseInt($scope.currentProduct.VPO_P_ID),
            VPO_ACC_ID: $scope.currentProduct.VPO_ACC_ID ? parseInt($scope.currentProduct.VPO_ACC_ID) : null,
            PRODUCT_NAME: $scope.currentProduct.PRODUCT_NAME,
            QUANTITY: $scope.currentProduct.PART_QTY,
            PRICE: $scope.currentProduct.PART_PRICE
        };

        // Call the update service method
        var datalist = VendorPOViewService.Admin_Update(updatedProduct);
        datalist.then(function (d) {
            if (d.data.success === true) {
                alert("Product updated successfully.");
                $('#UpdateProductAccessories').modal('hide'); // Close modal on success
                GetVendorPOProductList(); // Refresh product list
            } else {
    ...
    $scope.openUpdateModal = function (product) {
        ...
        // Set update mode and clone the product object
        $scope.isUpdateMode = true;
        $scope.currentProduct = angular.copy(product) || {};

        // Log the values of VPO_P_ID and CUSTOMER_TYPE_ID
        console.log("Product ID (VPO_P_ID):", product.VPO_P_ID);
        console.log("Customer Type ID:", product.CUSTOMER_TYPE_ID);

        // Reset accessories
        $scope.currentProduct.accessories = [];

        // Ensure product ID and customer type ID are defined before fetching accessories
        if (product.VPO_P_ID && product.CUSTOMER_TYPE_ID) {
            loadAccessoriesByProductId(product.VPO_P_ID, product.CUSTOMER_TYPE_ID);
        } else {
            console.warn("Product ID or Customer Type ID is missing.");
        }

        // Open the modal
        $('#UpdateProductAccessories').modal('show');
    };


    // Load accessories by product ID and customer type ID
    function loadAccessoriesByProductId(productId, customerTypeId) {
        if (!productId || !customerTypeId) {
            console.error("Missing productId or customerTypeId in loadAccessoriesByProductId.");
            return;
        }

        VendorPOViewService.GetAccessoriesByProductId(productId, customerTypeId)
            .then(function (response) {
                if (response.data) {
                    $scope.ProductAccessoriesSparePartList = response.data;
                    $scope.currentProduct.accessories = response.data;
                    console.log("Accessories fetched:", $scope.ProductAccessoriesSparePartList);
                } else {
                    console.warn("No accessories returned for the given product.");
                }
            })
    ...
    $scope.ProductAccessoriesSparePartList = [
        ...
        { ACC_ID: 1, ACCESSORY_NAME: 'Accessory 1', ACC_TYPE_ID: 1 },
        { ACC_ID: 2, ACCESSORY_NAME: 'Accessory 2', ACC_TYPE_ID: 2 },
        // Add other accessories here
    ];

    // Function to check all standard accessories
    $scope.checkAllStandardAccessories = function () {
        const allSelected = $scope.ProductAccessoriesSparePartList.filter(acc => acc.ACC_TYPE_ID === 1);
        const allChecked = allSelected.every(acc => acc.selected);
        allSelected.forEach(acc => acc.selected = !allChecked);
    };

    // Ensure to initialize quantities if needed
    $scope.ProductAccessoriesSparePartList.forEach(acc => {
        acc.quantity = 0; // Or any default value you want
    });
    ...
    $scope.updateSelectedAccessories = function () {
        const selectedAccessories = $scope.ProductAccessoriesSparePartList.filter(acc => acc.selected);
        // Proceed with the update logic
        console.log(selectedAccessories); // Debug the selected items
    };
