...
app.controller("RegularQuotationCtrl", function ($scope, QuotationService) {
    ...
    $scope.ShowHideStdAcc = function () {
        ...
        const stdAccessoriesCheckbox = document.getElementById("STDACCESSORIES");
        const stdAccTable = document.getElementById("StdAccTB");

        if (stdAccessoriesCheckbox.checked === true) {
            const value = $('.STD_ACC:checked').val();
            console.log(value);

            if ($scope.StdAccPartList && $scope.StdAccPartList.length > 0) {
                $scope.IS_WITH_STANDARD_ACC = "Yes";
                stdAccTable.style.display = "block";
            } else {
                alert("No Standard Accessories available for this product!");
                $scope.IS_WITH_STANDARD_ACC = "No";
                stdAccTable.style.display = "none";
                stdAccessoriesCheckbox.checked = false;
            }
        } else {
            $scope.IS_WITH_STANDARD_ACC = "No";
            stdAccTable.style.display = "none";
        ...
        console.log("IS_WITH_STANDARD_ACC:", $scope.IS_WITH_STANDARD_ACC);
    };
