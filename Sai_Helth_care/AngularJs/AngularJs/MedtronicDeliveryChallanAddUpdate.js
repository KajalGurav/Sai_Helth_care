...
app.controller("AddUpdateDeliveryChallanCtrl", function ($scope, DeliveryChallanService) {
    ...
    $scope.OnCustomerChange = function () {
        if ($scope.Customer_ID === undefined || $scope.Customer_ID === null || $scope.Customer_ID === "") {
            ...
            $scope.SelectedFirmName = ""; // Clear the firm name when no customer is selected
        } else {
            GetAllCustomerFirm();
        }
    };
    function GetAllCustomerFirm() {
        ...
        getAdmin.then(function (response) {
            ...
            if ($scope.CustomerFirmList.length > 0) {
                // Automatically pick the first firm's name and set it in the readonly input
                $scope.SelectedFirmName = $scope.CustomerFirmList[0].FIRM_NAME;
            } else {
                $scope.SelectedFirmName = ""; // Handle cases where no firm is found
            }
        });
    }
