...
app.controller("AddUpdateMedtronicInvoiceCtrl", function ($scope, InvoiceService) {
    ...
    $scope.loadCustomerFirms = function () {
        InvoiceService.GetCustomerFirmList($scope.Customer_ID).then(function (response) {
            $scope.CustomerFirmList = response.data;
        }, function (error) {
            console.log("Error while loading customer firms: ", error);
        });
    };
    ...
    function GetAllCustomers() {
        return InvoiceService.GetAllCustomer($scope.CUSTOMER_TYPE_ID).then(function (response) {
            $scope.AllCustomerList = response.data;
            console.log("Customers fetched successfully:", $scope.AllCustomerList);
        }, function (error) {
            console.error("Error fetching customers:", error);
        });
    }
    if ($scope.PAGE_NAME === 'Customer') {
        // Ensure customer name and firm name are auto-filled
        if ($scope.Customer_ID) {
            // Fetch all customers and set the customer name
            GetAllCustomers().then(() => {
                const selectedCustomer = $scope.AllCustomerList.find(cust => cust.Customer_ID === $scope.Customer_ID);
                if (selectedCustomer) {
                    $scope.CustomerName = selectedCustomer.CUSTOMER_NAME; // Auto-fill customer name
                    console.log("Customer Name auto-filled:", $scope.CustomerName);

                    // Fetch and auto-fill firms for this customer
                    GetAllCustomerFirm($scope.Customer_ID);
                } else {
                    console.warn("No customer found for Customer_ID:", $scope.Customer_ID);
                }
            });
        } else {
            console.warn("Customer_ID is missing for page 'customer'.");
        }
    }

    $scope.OnCustomerChange = function () {
        const selectedCustomerName = $scope.CustomerName;

        if (selectedCustomerName) {
            // Find the selected customer
            const selectedCustomer = $scope.AllCustomerList.find(cust => cust.CUSTOMER_NAME === selectedCustomerName);

            if (selectedCustomer) {
                $scope.Customer_ID = selectedCustomer.Customer_ID;
                console.log("Customer ID found:", $scope.Customer_ID);

                // Fetch firms for the selected customer
                GetAllCustomerFirm($scope.Customer_ID);
            } else {
                console.warn("No matching customer found for name:", selectedCustomerName);
                resetFirmSelection();
            }
        } else {
            resetFirmSelection();
    ...
    function resetFirmSelection() {
        $scope.SelectedFirmName = "";
        $scope.CustomerFirmList = [];
        $scope.F_ID = "";
    }

    function GetAllCustomerFirm(customerID) {
        if (!customerID) {
            console.warn("No Customer ID provided for fetching firm.");
            return;
        }
        InvoiceService.GetFirmList(customerID).then(function (response) {
            console.log("Firms fetched: ", response.data);
            $scope.CustomerFirmList = response.data;

            if ($scope.CustomerFirmList.length > 0) {
                $scope.F_ID = $scope.CustomerFirmList[0].F_ID;  // Auto-select the first firm
                $scope.SelectedFirmName = $scope.CustomerFirmList[0].FIRM_NAME;  // Display firm name
                console.log("Firm Name auto-filled:", $scope.SelectedFirmName);
            } else {
                resetFirmSelection();
            }
        }, function (error) {
    ...
    // Watcher to call OnCustomerChange when Customer_ID changes
    $scope.$watch('Customer_ID', function (newVal) {
        if (newVal) {
