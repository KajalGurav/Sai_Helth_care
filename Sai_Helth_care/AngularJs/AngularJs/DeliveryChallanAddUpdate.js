...
app.controller("AddUpdateDeliveryChallanCtrl", function ($scope, DeliveryChallanService) {
    ...
    function GetAllCustomers() {
        return DeliveryChallanService.GetAllCustomer($scope.CUSTOMER_TYPE_ID).then(function (response) {
            $scope.AllCustomerList = response.data;
            console.log("Customer List Loaded:", $scope.AllCustomerList);
        });
    }

    // Sample customer data with Customer_ID, CUSTOMER_NAME, and FIRM_NAME


    // Function to update firm name based on selected customer
    $scope.updateFirmName = function () {
        // Find the selected customer by name
        const selectedCustomer = $scope.AllCustomerList.find(
            cust => String(cust.Customer_ID) === String($scope.Customer_ID)
        );

        if (selectedCustomer) {
            // If a customer is found, set the customer ID and firm name
            $scope.Customer_ID = selectedCustomer.CUSTOMER_ID;
            $scope.selectedFirmName = selectedCustomer.FIRM_NAME; // Ensure FIRM_NAME is a property of your customer object
        } else {
            // If no customer is found, clear the ID and firm name
    ...
    $scope.OnCustomerChange = function () {
        if ($scope.Customer_ID) {
            const selectedCustomer = $scope.AllCustomerList.find(cust => cust.Customer_ID === $scope.Customer_ID);

            if (selectedCustomer) {
                $scope.customerName = selectedCustomer.CUSTOMER_NAME; // Update Customer Name
            } else {
                $scope.customerName = ''; // Clear Customer Name if no match is found
            }

            GetAllCustomerFirm();
        } else {
            $scope.CustomerFirmList = [];
            $scope.customerName = ''; // Clear Customer Name
            $scope.selectedFirmName = ''; // Clear Firm Name
            $scope.F_ID = null; // Clear selected firm
        }
    };
    ...
    $scope.initPage = function () {
        ...
        const custId = queryParams.get("CustId");

        if (custId) {
            $scope.Customer_ID = custId; // Set Customer_ID
            $scope.autoSelectCustomerById(); // Populate Customer Name and Firm Name
        } else {
            GetAllCustomers(); // Fallback to load all customers
        }
    };

    $scope.autoSelectCustomerById = function () {
        GetAllCustomers();

        const interval = setInterval(() => {
            if ($scope.AllCustomerList && $scope.AllCustomerList.length > 0) {
                clearInterval(interval);

                console.log("Customer_ID:", $scope.Customer_ID);
                console.log("AllCustomerList:", $scope.AllCustomerList);

                // Find the matching customer by Customer_ID (with type conversion)
                const selectedCustomer = $scope.AllCustomerList.find(
                    cust => String(cust.Customer_ID) === String($scope.Customer_ID)
                );

                if (selectedCustomer) {
                    $scope.customerName = selectedCustomer.CUSTOMER_NAME;
                    $scope.selectedFirmName = selectedCustomer.FIRM_NAME;

                    document.getElementById("Customer_ID").setAttribute("readonly", true);
                    document.getElementById("selectedFirmName").setAttribute("readonly", true);
                } else {
                    console.warn("Customer not found for Customer_ID:", $scope.Customer_ID);
                    $scope.customerName = '';
                    $scope.selectedFirmName = '';
                }
            }
        }, 100); // Retry until customer list is available
    };
    ...
    function GetAllCustomerFirm() {
        ...
        getAdmin.then(function (response) {
            ...
            if ($scope.CustomerFirmList.length > 0) {
                $scope.F_ID = $scope.CustomerFirmList[0].F_ID;
                $scope.selectedFirmName = $scope.CustomerFirmList[0].FIRM_NAME;
            } else {
                $scope.F_ID = null;
                $scope.selectedFirmName = '';
            }
